            ...
            <div className="ml-7 border-l-2 border-gradient-violet pl-10 space-y-8 border-violet-300 dark:border-violet-700">
              {exp.projects.map((project, index) => {
                ...
                const card = (
                  <div
                    className={cn(
                      "group relative h-full space-y-3 p-5 rounded-xl bg-card border border-border/50 hover:shadow-md hover:border-violet-200 dark:hover:border-violet-500/30 transition-all duration-500",
                      isVisible
                  ...
                  >
                    ...
                    <div className="flex items-start gap-1.5">
                      <h4
                        className={cn(
                          "font-semibold text-lg underline-offset-4",
                      ...
                      {slug && (
                        <ArrowUpRight className="mt-1 h-4 w-4 shrink-0 text-violet-500 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
                      )}
                    </div>
                ...
                // Whole card is the click target, like the project cards.
                // Deliveries without a detail page stay plain.
                return slug ? (
                  <Link
                    key={project.name}
                    href={`/projects/${slug}`}
